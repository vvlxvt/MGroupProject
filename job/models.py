...
class Post(models.Model):
    ...
    )  # чтобы добавить поле надо сделать сначала с null=True, потом без него
    photo = models.ImageField(
        upload_to=upload_to, default=None, blank=True, null=True, verbose_name="Фото"
    )
    articles = models.ManyToManyField(
        "Article", through="PostArticle", related_name="posts"
    )
    ...
    def get_absolute_url(self):
        ...
        return reverse("job:post_detail", args=[self.slug])
...
class Article(models.Model):
    ...
    objects = models.Manager()
    photo = models.ImageField(
        upload_to=upload_to, max_length=255, default=None, blank=True, null=True
    )
    ...
    def get_absolute_url(self):
        # возвращает канонический URL-адрес объекта
        return reverse("job:article_detail", args=[self.slug])
